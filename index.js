...
var keyBlacklist = ["__proto__", "constructor", "prototype"];

exports = module.exports = function (url, cb, options) {
  var userAgent =
    (options || {}).userAgent ||
    "NodeOpenGraphCrawler (https://github.com/samholmes/node-open-graph)";
...
exports.getHTML = function (url, userAgent, cb) {
  ...
  if (!purl.protocol) purl = require("url").parse("http://" + url);
  ...
  request(
    ...
    function (err, res, body) {
      ...
      if (res.statusCode === 200) {
        ...
        cb(
          new Error("Request failed with HTTP status code: " + res.statusCode)
        );
      }
    }
  );
...
exports.parse = function ($, options) {
  ...
  metaTags.each(function () {
    ...
    if (
      !propertyAttr ||
      propertyAttr.substring(0, namespace.length) !== namespace
    )
      return;
